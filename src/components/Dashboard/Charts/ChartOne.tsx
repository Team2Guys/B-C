  ...
  const getMonthlyRecord = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/admins/getMonthlySales`,
        {
          headers: {
            token: finalToken,
          },
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 sm:flex-nowrap">
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-48">
            <span className="mr-2 mt-1 flex h-4 w-full max-w-4 items-center justify-center rounded-full border border-black dark:border-white">
              <span className="block h-2 w-full max-w-2 rounded-full bg-black dark:bg-white"></span>
            </span>
        ...
        <div>
          <button className="rounded px-3 py-1 text-xs font-medium text-black hover:bg-primary hover:text-white hover:shadow-card dark:text-white dark:hover:bg-dashboardDark">
            Month
          </button>
        </div>
      </div>
