  ...
  const getWeeklySales = async () => {
    try {
      ...
      let response: any = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/admins/getWeeklySales`,
        {
          headers: {
            token: finaltoken,
          },
      ...
      <div className="mb-4 justify-between gap-4 sm:flex">
        ...
        <div>
          <div className="relative z-20 inline-block">
            <p className="inline-flex appearance-none bg-transparent py-1 pl-3 pr-8 text-sm font-medium outline-none text-black dark:text-white">
              {' '}
              Previous Week
            </p>
          </div>
