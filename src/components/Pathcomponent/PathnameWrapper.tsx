  ...
  return (
    <>
      ...
      {withoutHeaderPages.includes(pathname) || splited_urls.includes('dashboard') ? null : (
        <>
        {splited_urls.includes('blog') || splited_urls.includes('product-guarantees') || pathname === '/faqsnew/' || isNotFoundPage  || splited_urls.includes('request-appointment') || splited_urls.includes('estimator') ? null : (
