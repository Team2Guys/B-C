...
const Estimator: React.FC = () => {
  ...
  useEffect(() => {
    if (categories && products) {
      ...
      const firstCategory = categories.find(category => category.title.toLowerCase().includes('blinds'));
      
      if (firstCategory) {
        const productsInFirstCategory = products.filter(product => product.CategoryId === firstCategory.id);
      ...
      <div className="mt-10 lg:max-w-[95%] 2xl:max-w-[90%]">
        <div className="flex flex-wrap md:flex-nowrap gap-10">
          ...
          <div className="flex flex-col space-y-5 w-full md:w-5/12 px-2 md:px-0">
            ...
            <p className="lg:text-[35px] text-2xl font-bold">AED {selectedProduct?.price}</p>
            <p className="lg:text-[15px] text-md">{selectedProduct?.description} </p>
